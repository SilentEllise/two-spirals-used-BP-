# Twin Spirals classification, main module
...
def train(net, train_loader, optimizer):
    ...
    if epoch % 1000 == 0:
        print('epoch:%5d loss: %6.4f precision: %5.2f' % (epoch, loss.item(), accuracy))

    return accuracy

def graph_output(net):
    xrange = torch.arange(start=-6.1, end=6.1, step=0.02, dtype=torch.float32)
    yrange = torch.arange(start=-6.1, end=6.1, step=0.02, dtype=torch.float32)
    xaxis = xrange.repeat(yrange.size()[0])
    yaxis = torch.repeat_interleave(yrange, xrange.size()[0], dim=0)
    grid = torch.cat((xaxis.unsqueeze(1), yaxis.unsqueeze(1)), 1)
    ...
    with torch.no_grad(): # suppress updating of gradients
        ...
        plt.clf()
        plt.pcolormesh(xrange, yrange, pred.cpu().view(yrange.size()[0], xrange.size()[0]), shading='auto', cmap='summer')
...
if list(net.parameters()):
    ...
    # use Adam optimizer
    optimizer = torch.optim.Adam(net.parameters(), eps=0.0001, lr=args.lr,
                                 betas=(0.9, 0.999), weight_decay=0.0001)

    # training loop
    for epoch in range(1, args.epochs):
        accuracy = train(net, train_loader, optimizer)
        if epoch % 1000 == 0 and accuracy == 100:
            break
...
for layer in [1, 2]:
    if layer == 1 or args.net != 'polar':
        for node in range(args.hid):
            ...
            plt.scatter(full_input[:, 0], full_input[:, 1], c=1-full_target[:, 0], cmap='YlGnBu')

# graph output unit
graph_output(net)
plt.scatter(full_input[:, 0], full_input[:, 1],
            c=1-full_target[:, 0], cmap='YlGnBu')
